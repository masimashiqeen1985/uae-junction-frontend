...
export function VibrantHome() {
  ...
  useEffect(() => {
    ...
    // ---- Hotels & Flights enquiry: functional tabs + real submit (WhatsApp / email) ----
    // Manual lead-gen (no online checkout for hotels/flights): we pre-fill a quote
    // message and hand off to WhatsApp or email so the UAE travel desk replies by hand.
    const SALES_EMAIL = 'sales@theuaejunction.com'
    const WHATSAPP = '971585898221' // +971 58 589 8221
    ...
    // set min date = today on the native date pickers
    ...
    enqTabs.forEach((t) => t.addEventListener('click', () => {
      ...
      const destInput = enqForm?.querySelector<HTMLInputElement>('input[name="destination"]')
      if (destInput) destInput.placeholder = enqType === 'Flight' ? 'Dubai → Istanbul' : 'Dubai, Istanbul…'
    }))

    if (enqForm) enqForm.addEventListener('submit', (e) => {
      e.preventDefault()
      const field = (n: string) => enqForm.querySelector<HTMLInputElement | HTMLSelectElement>('[name="' + n + '"]')
      const destEl = field('destination') as HTMLInputElement | null
      const travEl = field('travellers') as HTMLSelectElement | null
      const inEl = field('checkin') as HTMLInputElement | null
      const outEl = field('checkout') as HTMLInputElement | null
      const contactEl = field('contact') as HTMLInputElement | null
      ...
      const destInput = enqForm.querySelector<HTMLInputElement>('input[name="destination"]')
      if (destInput) destInput.placeholder = 'Dubai, Istanbul…'
    })
    ...
    // forms -> prevent submit + toast (the real Hotels & Flights enquiry form is handled above)
    Array.from(r.querySelectorAll<HTMLFormElement>('form')).forEach((fm) => {
      if (fm.classList.contains('enq-form')) return
