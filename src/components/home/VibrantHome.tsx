...
export function VibrantHome({ destinations = [] }: { destinations?: HeroCountry[] }) {
  ...
  useEffect(() => {
    ...
    enqTabs.forEach((t) => t.addEventListener('click', () => {
      ...
      if (di) di.placeholder = enqType === 'Flight' ? 'Dubai â Istanbul' : 'Dubai, Istanbulâ¦'
    }))
    if (enqForm) enqForm.addEventListener('submit', (ev) => {
      ...
      const fmt = (v?: string) => v ? new Date(v + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'â'
      const isFlight = enqType === 'Flight'
      const lines = [
        'Hello UAE Junction â Iâd like a quote.',
        'Type: ' + enqType,
        (isFlight ? 'Route: ' : 'Destination: ') + destination,
        'Travellers: ' + (travEl?.value || 'â'),
        (isFlight ? 'Depart: ' : 'Check-in: ') + fmt(inEl?.value),
        (isFlight ? 'Return: ' : 'Check-out: ') + fmt(outEl?.value),
        'Contact: ' + contact,
        '',
        '(I understand hotels & flights are hand-priced, and Iâll earn 2.5% cashback once booked.)',
      ]
      const body = lines.join('\n')
      const isEmail = /^[^\s\u0040]+\u0040[^\s\u0040]+\.[^\s\u0040]+$/.test(contact)
      if (isEmail) {
        const subject = encodeURIComponent('Quote request â ' + enqType + ' â ' + destination)
      ...
      toast('â Opening your pre-filled enquiry â just hit send!')
      enqForm.reset()
      enqType = 'Hotel'
      ...
      if (di2) di2.placeholder = 'Dubai, Istanbulâ¦'
    })
  ...
  return (
    <div className="vibrant-home" ref={root}>
      ...
      <button type="button" className="enq-tab active" data-type="Hotel" aria-pressed="true">ð¨ Hotel</button>
      <button type="button" className="enq-tab" data-type="Flight" aria-pressed="false">âï¸ Flight</button>
      <button type="button" className="enq-tab" data-type="Both" aria-pressed="false">ð§³ Both</button>
      </div>
      <div className="enq-row">
      <label>Destination<input name="destination" type="text" placeholder="Dubai, Istanbulâ¦" required /></label>
      <label>Travellers
      ...
      <label className="enq-full">Your WhatsApp or email<input name="contact" type="text" inputMode="email" placeholder="+971 50â¦ or you@email.com" required /></label>
      <button type="submit" className="btn btn-grad enq-submit">Request my quote â</button>
