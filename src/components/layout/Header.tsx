...
const NAV=[{label:'Promotions',href:'/promotions'},{label:'About Us',href:'/about-us'},{label:'Packages & Offers',href:'#',children:[{label:'Theme Parks',href:'/theme-parks'},{label:'Water Parks',href:'/water-parks'},{label:'Desert Safari',href:'/desert-safari'},{label:'Dhow Cruise',href:'/dhow-cruise'},{label:'Experiences',href:'/experiences'},{label:'UAE City Tours',href:'/uae-city-tours'},{label:'Hotel Booking',href:'/hotel-booking'},{label:'Flight Booking',href:'/flight-booking'},{label:'Umrah Packages',href:'/umrah-packages'}]},{label:'Contact Us',href:'/contact-us'},{label:'Blogs',href:'/blogs'}]
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm">
            ...
            {NAV.map(item=>item.children?(
              <div key={item.label} className="relative group">
                ...
                <div className="absolute top-full left-0 mt-2 w-52 bg-white rounded-card shadow-card-hover py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  {item.children.map(c=><Link key={c.href} href={c.href} className="block px-4 py-2 text-sm text-neutral-700 hover:text-primary hover:bg-neutral-50">{c.label}</Link>)}
                </div>
              </div>
            ):(
      ...
      {open&&<div className="lg:hidden bg-white border-t border-neutral-100 px-4 py-4 space-y-2">
        {NAV.map(item=>item.children?(
          ...
          <div className="pl-4 mt-1 space-y-1">{item.children.map(c=><Link key={c.href} href={c.href} className="block py-1.5 text-sm text-neutral-600 hover:text-primary" onClick={()=>setOpen(false)}>{c.label}</Link>)}</div></details>
        ):(
