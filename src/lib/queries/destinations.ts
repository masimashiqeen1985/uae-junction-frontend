...
export const GET_DESTINATION_PRODUCTS=`query DestinationProducts($slug:ID!,$first:Int=100){destination(id:$slug,idType:SLUG){name slug description count parent{node{name slug}}children{nodes{name slug count}}products(first:$first,where:{visibility:VISIBLE}){nodes{id databaseId slug name shortDescription onSale image{${MEDIA_FIELDS}} ... on SimpleProduct{regularPrice salePrice price} productCategories{nodes{name slug}}}}}}`
