...
async function getData():Promise<CatData>{
  try{return await fetchGraphQL<CatData>(GET_CATEGORY_PRODUCTS,{slug:'umrah-packages',first:48},3600)}
  catch{return{}}
}
...
export default async function Page(){
  ...
  const breadcrumbLd={
    ...
    itemListElement:[
      ...
      {'@type':'ListItem',position:2,name:'Umrah Packages',item:'https://www.theuaejunction.cloud/umrah-packages'},
    ],
  }
  ...
  return(
    <div>
      ...
      <PageHero
        title="Umrah Packages"
        subtitle="Comfortable, well-organised Umrah journeys with trusted partners."
      />

      {products.length>0?(
        <div className="container-xl py-10 sm:py-12">
          <p className="text-sm text-neutral-500 mb-6" aria-live="polite">{products.length} umrah package{products.length===1?'':'s'}</p>
        ...
        <div className="container-xl py-16 text-center">
          <p className="text-neutral-500">Umrah packages are coming soon — check back shortly, or get a free quote below.</p>
        </div>
      )}
