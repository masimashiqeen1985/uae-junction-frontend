...
async function getData():Promise<CatData>{
  try{return await fetchGraphQL<CatData>(GET_CATEGORY_PRODUCTS,{slug:'dhow-cruise',first:48},3600)}
  catch{return{}}
}
...
export default async function Page(){
  ...
  const breadcrumbLd={
    ...
    itemListElement:[
      ...
      {'@type':'ListItem',position:2,name:'Dhow Cruise',item:'https://www.theuaejunction.cloud/dhow-cruise'},
    ],
  }
  ...
  return(
    <div>
      ...
      <PageHero
        title="Dhow Cruise"
        subtitle="Sail the Waters of Dubai"
      />

      {products.length>0?(
        <div className="container-xl py-10 sm:py-12">
          <p className="text-sm text-neutral-500 mb-6" aria-live="polite">{products.length} dhow cruise{products.length===1?'':'s'}</p>
        ...
        <div className="container-xl py-16 text-center">
          <p className="text-neutral-500">Dhow cruises are coming soon — check back shortly, or get a free quote below.</p>
        </div>
      )}
